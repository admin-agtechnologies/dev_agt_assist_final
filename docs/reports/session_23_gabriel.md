...
- Architecture décidée :
  ...
  - Stats = filtrées par actions autorisées du bot (nuance Gabriel)
...
**Diff manuel appliqué par Gabriel dans `apps/bots/views.py` :**
...
**Diff manuel appliqué par Gabriel dans `src/repositories/bots.repository.ts` :**
...
| Accordéon pour les sections config | Gabriel : sidebar ne suit plus + scroll propre + groupement logique |
