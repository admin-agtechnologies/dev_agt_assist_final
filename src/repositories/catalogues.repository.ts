    ...
    api
      ...
      .then((data: unknown) =>
        Array.isArray(data)
          ...
          : ((data as PaginatedResponse<QuestionFrequente>).results ?? []),
      ),
  ...
  getMine: (): Promise<TenantKnowledge | null> =>
    api
      ...
      .then((data: unknown) => {
        ...
        return pag.results && pag.results.length > 0
          ? pag.results[0]
          : null;
      })
      .catch(() => null),
  create: (
    payload: CreateTenantKnowledgePayload,
...
export const serviceKnowledgeRepository = {
  ...
  create: (
    payload: CreateServiceKnowledgePayload,
  ...
  getList: (): Promise<HelpEntry[]> =>
    api
      ...
      .then((data: unknown) =>
        Array.isArray(data)
          ? (data as HelpEntry[])
          : ((data as PaginatedResponse<HelpEntry>).results ?? []),
      ),
};