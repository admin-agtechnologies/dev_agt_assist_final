...
export const subscriptionsRepository = {
  ...
  getByTenant: (tenantId: string): Promise<Subscription | null> =>
    api
      .get("/api/v1/subscriptions", { params: { tenant_id: tenantId } })
      .then((data: unknown) =>
        Array.isArray(data) && data.length > 0
          ? (data[0] as Subscription)
          : null,
      ),
...
export const walletsRepository = {
  getByTenant: (tenantId: string): Promise<Wallet | null> =>
    api
      .get("/api/v1/wallets", { params: { tenant_id: tenantId } })
      .then((data: unknown) =>
...
export const faqRepository = {
  getList: (tenantId?: string): Promise<PaginatedResponse<FAQ>> =>
    api
      .get("/api/v1/faq", { params: tenantId ? { tenant_id: tenantId } : {} })
      .then((data: unknown) =>
        Array.isArray(data)
          ? {
              results: data as FAQ[],
...
export const tenantKnowledgeRepository = {
  getByTenant: (tenantId: string): Promise<TenantKnowledge> =>
    api
      .get("/api/v1/tenant-knowledge", { params: { tenant_id: tenantId } })
      .then((data: unknown) =>
        Array.isArray(data)
          ? (data[0] as TenantKnowledge)
          : (data as TenantKnowledge),
      ),
...
export const transactionsRepository = {
  getByTenant: (tenantId: string): Promise<Transaction[]> =>
    api
      .get("/api/v1/transactions", { params: { tenant_id: tenantId } })
      .then((data: unknown) =>
