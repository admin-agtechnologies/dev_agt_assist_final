// src/app/(dashboard)/billing/_components/ChangePlanModal.tsx
// Migration de src/app/pme/billing/components/ChangePlanModal.tsx
// Corrections : catch (err: any) → instanceof Error | useLanguage hook
"use client";
...
export function ChangePlanModal({
  plan, currentSub, wallet, onClose, onSuccess,
  ...
  const handleConfirm = async () => {
    ...
    setSaving(true);
    try {
      await billingActionsRepository.confirmUpgrade(plan.id);
      toast.success(t.changePlanSuccess);
      onSuccess();
    } catch (err) {
      const msg = err instanceof Error ? err.message : t.changePlanError;
      toast.error(msg);
    } finally {
      setSaving(false);
    }
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
      ...
      <div className="relative bg-[var(--bg-card)] rounded-3xl w-full max-w-md shadow-2xl border border-[var(--border)] animate-zoom-in">
        ...
        <div className="p-6 space-y-4">
          <div className="flex items-center gap-3">
            <div className="flex-1 bg-[var(--bg)] rounded-2xl p-4 text-center">
              ...
              <p className="text-lg font-black text-[var(--text)]">{currentSub.plan?.nom ?? "—"}</p>
              <p className="text-sm text-[var(--text-muted)]">{formatCurrency(currentSub.plan?.prix ?? 0)}</p>
            </div>
            ...
            <div className="flex-1 bg-[#25D366]/10 border-2 border-[#25D366] rounded-2xl p-4 text-center">
              ...
              <p className="text-lg font-black text-[var(--text)]">{plan.nom}</p>
              <p className="text-sm font-bold text-[#075E54]">{formatCurrency(plan.prix)}</p>
            </div>
          </div>
          ...
          <div className="bg-[var(--bg)] rounded-2xl p-4 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold text-[var(--text)]">{formatCurrency(wallet.solde)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-[var(--text-muted)]">{t.changePlanCost}</span>
              <span className="font-semibold text-red-500">−{formatCurrency(plan.prix)}</span>
            </div>
            <div className="flex justify-between text-sm border-t border-[var(--border)] pt-2 mt-1">
              ...
              <span className={cn("font-black", canAfford ? "text-[#075E54]" : "text-red-500")}>
          ...
          {!canAfford && (
            <div className="flex items-center gap-2 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800">
              ...
              <p className="text-xs text-amber-700">{t.changePlanInsufficient}</p>
            </div>
          )}
        </div>
        ...
        <div className="p-5 border-t border-[var(--border)] flex justify-end gap-3">
          ...
          <button onClick={handleConfirm} disabled={saving || !canAfford} className={cn("btn-primary", !canAfford && "opacity-50")}>
