...
const EMPTY = { depart_fr: "", destination_fr: "", tarif: "", duree_estimee: "", horaires_depart: "" };
...
export function CatalogueTrajetTab() {
  ...
  const toPayload = (f: typeof EMPTY) => ({
    depart_fr:      f.depart_fr.trim(),
    destination_fr: f.destination_fr.trim(),
    tarif:          f.tarif ? Number(f.tarif) : null,
    duree_estimee:  f.duree_estimee.trim() || undefined,
  ...
  const handleCreate = () => startSave(async () => {
    if (!form.depart_fr.trim() || !form.destination_fr.trim() || !form.tarif) return;
    try {
  ...
  const handleToggle = (item: CatalogueTrajet) => startSave(async () => {
    try {
      const u = await catalogueTrajetRepository.update(item.id, { is_available: !item.is_available });
  ...
  const startEdit = (item: CatalogueTrajet) => {
    ...
    setShowAdd(false);
    setForm({
      depart_fr:      item.depart_fr,
      destination_fr: item.destination_fr,
      tarif:          item.tarif != null ? String(item.tarif) : "",
      duree_estimee:  item.duree_estimee ?? "",
      horaires_depart: item.horaires_depart.join(", "),
    });
  };
  ...
  return (
    <div className="space-y-5">
      ...
      {showAdd && (
        <TrajetForm form={form} set={set}
          onSave={handleCreate} onCancel={() => setShowAdd(false)}
          saving={saving} theme={theme} label="Nouveau trajet" />
      )}
                ...
                <div className="flex items-center gap-3 mt-1.5">
                  {item.tarif != null && (
                    <span className="text-sm font-bold" style={{ color: theme.primary }}>
                      {Number(item.tarif).toLocaleString("fr-FR")} XAF
                    </span>
                  )}
                  {item.duree_estimee && (
                    <span className="flex items-center gap-1 text-xs text-[var(--text-muted)]">
                      <Clock className="w-3 h-3" />{item.duree_estimee}
                    </span>
                  )}
                </div>
  ...
  return (
    <div className="bg-[var(--bg-card)] rounded-2xl border-2 p-5 space-y-3"
      ...
      <div className="grid grid-cols-2 gap-3">
        <Row label="Départ *">
          <input className="input-base" value={form.depart_fr} onChange={set("depart_fr")} placeholder="Douala" autoFocus />
        </Row>
        <Row label="Destination *">
          <input className="input-base" value={form.destination_fr} onChange={set("destination_fr")} placeholder="Yaoundé" />
        </Row>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <Row label="Tarif (XAF) *">
          <input className="input-base" type="number" min="0" value={form.tarif} onChange={set("tarif")} />
        </Row>
        <Row label="Durée (ex: 4h, 1h30)">
          <input className="input-base" placeholder="4h" value={form.duree_estimee} onChange={set("duree_estimee")} />
        </Row>
      </div>
      <Row label="Horaires (séparés par virgule)">
        <input className="input-base" placeholder="06:00, 09:00, 14:00"
          value={form.horaires_depart} onChange={set("horaires_depart")} />
      </Row>
      <div className="flex justify-end gap-2 pt-1">
        ...
        <button type="button" onClick={onSave}
          disabled={saving || !form.depart_fr.trim() || !form.destination_fr.trim() || !form.tarif}
