...
export function MedicalTab() {
  ...
  // "Dr. Mballa (Cardiologue), Dr. Ngo (Généraliste)" → [{nom, titre}]
  const parseMedecins = (s: string) =>
    s.split(",").map((m) => m.trim()).filter(Boolean).map((m) => {
      const match = m.match(/^(.+?)\s*\((.+?)\)$/);
      return match
        ? { nom: match[1].trim(), titre: match[2].trim() }
        : { nom: m, titre: "" };
    });
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        {!showAdd && (
          <button type="button"
            ...
            <Plus className="w-4 h-4" /> Ajouter une spécialité
          </button>
        )}
      </div>

      {showAdd && (
        <SpecialiteForm form={form} set={set}
          onSave={handleCreate} onCancel={() => setShowAdd(false)}
          saving={saving} theme={theme} label="Nouvelle spécialité" />
      )}
      ...
      {items.length === 0 && !showAdd ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-center
          ...
          <p className="text-sm text-[var(--text-muted)]">Aucune spécialité configurée.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {items.map((item) => editId === item.id ? (
            <div key={item.id} className="sm:col-span-2 xl:col-span-3">
              <SpecialiteForm form={form} set={set}
                ...
                saving={saving} theme={theme} label="Modifier la spécialité" />
            </div>
          ) : (
                ...
                <div className="flex items-center gap-3 text-xs text-[var(--text-muted)]">
                  {item.medecins.length > 0 && (
                    <span className="flex items-center gap-1">
                      ...
                      {item.medecins.length} médecin{item.medecins.length > 1 ? "s" : ""}
                    </span>
                  )}
...
function SpecialiteForm({ form, set, onSave, onCancel, saving, theme, label }: {
  ...
  return (
    <div className="bg-[var(--bg-card)] rounded-2xl border-2 p-5 space-y-3"
      ...
      <Row label="Nom de la spécialité *">
      ...
      <Row label='Médecins (ex: "Dr. Mballa (Cardiologue), Dr. Ngo (Généraliste)")'>
        <input className="input-base"
          placeholder="Dr. Mballa (Cardiologue), Dr. Ngo (Généraliste)"
          value={form.medecins} onChange={set("medecins")} />
      </Row>
      <Row label="Mots-clés / symptômes (séparés par virgule)">
        <input className="input-base"
          placeholder="fièvre, douleur thoracique, essoufflement"
          value={form.mots_cles_symptomes} onChange={set("mots_cles_symptomes")} />
      </Row>
      <Row label="Contact d'urgence">
        <input className="input-base" placeholder="+237 6XX XXX XXX"
