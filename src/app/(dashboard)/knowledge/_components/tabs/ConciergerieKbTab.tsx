...
export function ConciergerieKbTab() {
  ...
  const startEdit = (item: CatalogueItemKB) => {
    ...
    setForm({ nom: item.nom, description: item.description ?? "", prix: item.prix != null ? String(item.prix) : "" });
  };
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <p className="text-sm text-[var(--text-muted)]">
          {items.length} {locale === "fr" ? `service${items.length !== 1 ? "s" : ""} de conciergerie` : `concierge service${items.length !== 1 ? "s" : ""}`}
        </p>
        {!showAdd && !editId && (
          <button type="button"
