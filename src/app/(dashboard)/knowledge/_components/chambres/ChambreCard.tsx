...
type FormState = {
  nom_fr: string; nom_en: string; description_fr: string;
...
function chambreToForm(c: ChambreType): FormState {
  return {
    nom_fr: c.nom_fr, nom_en: c.nom_en, description_fr: c.description_fr,
    capacite: String(c.capacite), prix_nuit: String(c.prix_nuit),
...
export function ChambreCard({ chambre, onUpdated, onDeleted, t }: Props) {
  ...
  const handleDelete = () => startSave(async () => {
    try { await chambreRepository.delete(chambre.id); onDeleted(chambre.id); toast.success(t.deleted); }
    ...
    return (
      <div className="bg-[var(--bg-card)] rounded-2xl border-2 p-5 space-y-3 col-span-1 sm:col-span-2 xl:col-span-3"
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <Row label={t.nomFr + " *"}><input className="input-base" value={form.nom_fr} onChange={set("nom_fr")} autoFocus /></Row>
          <Row label={t.nomEn}><input className="input-base" value={form.nom_en} onChange={set("nom_en")} /></Row>
        </div>
        <Row label={t.description}><textarea className="input-base resize-none" rows={2} value={form.description_fr} onChange={set("description_fr")} /></Row>
        <div className="grid grid-cols-2 gap-3">
          <Row label={t.capacite + " *"}><input className="input-base" type="number" min="1" value={form.capacite} onChange={set("capacite")} /></Row>
          <Row label={t.prixNuit + " *"}><input className="input-base" type="number" min="0" value={form.prix_nuit} onChange={set("prix_nuit")} /></Row>
        </div>
        <Row label={t.equipements}><input className="input-base" placeholder="WiFi, Clim, TV..." value={form.equipements} onChange={set("equipements")} /></Row>
        <Row label={t.imageUrl}><input className="input-base" type="url" placeholder="https://..." value={form.image_url} onChange={set("image_url")} /></Row>
        <div className="flex justify-end gap-2 pt-1">
          <button type="button" onClick={() => { setEditing(false); setForm(chambreToForm(chambre)); }}
          ...
          <button type="button" onClick={handleSave} disabled={saving || !form.nom_fr.trim() || !form.prix_nuit}
    ...
    <div className={cn(
      ...
      <div className="p-4 flex flex-col gap-2 flex-1">
        ...
        <div className="flex items-center gap-3 text-xs text-[var(--text-muted)]">
          <span className="flex items-center gap-1"><Users className="w-3 h-3" /> {chambre.capacite} pers.</span>
        ...
        {chambre.equipements.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {chambre.equipements.slice(0, 4).map((eq) => (
              <span key={eq} className="text-[10px] px-1.5 py-0.5 rounded-md bg-[var(--bg)] text-[var(--text-muted)]">
                {eq}
              </span>
            ))}
        ...
        <div className="flex justify-end gap-1 mt-auto pt-2">
          <button type="button" onClick={() => { setForm(chambreToForm(chambre)); setEditing(true); }}
            className="p-1.5 rounded-lg hover:bg-[var(--bg)] transition-colors">
            <Pencil className="w-3.5 h-3.5 text-[var(--text-muted)]" />
          </button>
          <button type="button" onClick={handleDelete} disabled={saving}
            className="p-1.5 rounded-lg hover:bg-red-50 transition-colors">
            {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5 text-red-400" />}
          </button>
        </div>
      </div>
    </div>
